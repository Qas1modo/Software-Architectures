...
public class UpdateAccessCardCommandHandler(IAccessCardRepository accessCardRepository, IAccessCardPermissionRepository accessCardPermissionRepository, IAccessCardRoleRepository accessCardRoleRepository, IUnitOfWork unitOfWork)
    : ICommandHandler<UpdateAccessCardCommand, Result>
{
    public async Task<Result> Handle(UpdateAccessCardCommand request, CancellationToken cancellationToken)
    {
        ...
        var permissionsToAdd = request.UpdateAccessCardModel.PermissionNamesToAdd;
        var permissionsToRemove = request.UpdateAccessCardModel.PermissionNamesToRemove;
        

        if (permissionsToAdd.Any())
        {
            await accessCardPermissionRepository.AddPermissionsToCardByName(accessCard.Value.Id, permissionsToAdd, cancellationToken);
        }

        if (permissionsToRemove.Any())
        {
            await accessCardPermissionRepository.RemovePermissionsFromCardByName(accessCard.Value.Id, permissionsToRemove, cancellationToken);
        }
